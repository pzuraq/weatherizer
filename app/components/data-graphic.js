// Copied and modified from ember-metrics-graphics
// https://github.com/jdjkelly/ember-metrics-graphics
...
export default Ember.Component.extend({
  ...
  renderData: function() {
    // Convert data to a standard array so that metrics graphics can read it
    ...
    if (data) {
      if (data.map) {
        data = data.map(function(item) {
          return {
            x: item.get(x_accessor),
            y: item.get(y_accessor)
          };
        });
      }
